		...
		*/
		...
		if($myAdmin->conexion->update(PREFIJO."contenido",$contenido,"WHERE fid_estructura=".$id)){
			if($myAdmin->conexion->update(PREFIJO."estructura",$datos,"WHERE kid_pagina=".$id)){
				header("Location:?est&edit&id=".$id."&msg=k1");
			}else{
				header("Location:?est&edit&id=".$id."&msg=e1");
			}
		}
	}else{
		header("Location:?est&edit&id=".$_POST['fid']);
	}
}else{/*Es guardar desde nuevo*/
/**
	kid_pagina
	nivel
	padre
...
#print_r($_POST);
	...
	$datos['nombre'] = $_POST['titulo'];
	$datos['nombre_EN'] = isset($_POST['nombre_EN'])?$_POST['nombre_EN']:'';
	$datos['nombre_FR'] = isset($_POST['nombre_FR'])?$_POST['nombre_FR']:'';
	$datos['alias'] = convertirDatoSeguro($_POST['alias']);
	...
	if(isset($_POST['type'])){
		if($_POST['type']=='inf' || $_POST['type']=='sup'){
			...
			foreach($registrosCambio as $reg){
				...
				$myAdmin->conexion->update(PREFIJO."estructura",$cambio,"WHERE kid_pagina=".$reg['kid_pagina']);
			}
	  ...
	  #die();
		...
		if($myAdmin->conexion->insert(PREFIJO."contenido",$contenido)){
			if(isset($_POST['newsite'])){/*es sitio- regresar a la estructura*/
				...
				header("Location:?est&edit&id=".$last."&msg=k1");
			}
		}
	}else{
		#echo $myAdmin->conexion->error;
		header("Location:?est&new&type=".$_POST['type']."&id=".$_POST['oldId']."&msg=e1&ertext=". $myAdmin->conexion->error);
	}
}
?>