...
if(		isset($_GET['new'])){ 		include_once("adm.$page.new.php");}
	...
	$queryT = "SELECT COUNT(*) as total FROM dato_reforma";
	...
	if($continuar){
		...
		$query = $myAdmin->conexion->query("SELECT * FROM dato_reforma ORDER BY fecha DESC LIMIT ".$paginacion['LIMIT']);
	}
	...
	foreach($arr as $reg){
		...
		#}else{
			...
			<tr class="<?php echo $reg['visible']==0?' inactivo':''?>" >
				<td><?php echo $reg['kid_dato']; ?></td>
				<td><?php echo html_entity_decode($reg['texto']); ?></td>
				<td style="white-space:nowrap"><?php echo $reg['fecha']; ?></td>
				<td><a href="?<?php echo $page; ?>&fast=<?php echo $reg['visible']==1?'invisible':'visible'; ?>&id=<?php echo $reg['kid_dato'] ?><?php echo $pager ?>" title="Hacer <?php echo $reg['visible']==1?'invisible':'visible'; ?>"><img src="img/ico/sem-<?php echo $reg['visible']==1?'vd':'gr'; ?>.gif" /></a></td>
				<td><a href="?<?php echo $page; ?>&edit&id=<?php echo $reg['kid_dato'] ?><?php echo $pager ?>" title="Editar tema"><img src="img/ico/edit.png" /></a></td>
				<td><a href="?<?php echo $page; ?>&delete&id=<?php echo $reg['kid_dato'] ?><?php echo $pager ?>" onclick="return confirmar();" title="Borrar tema"><img src="img/ico/delete.png" /></a></td>
			</tr>
			<?php
			$i++;
		}
