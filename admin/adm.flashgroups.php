...
$queryGroups = "SELECT COUNT(*) as total FROM ".PREFIJO."grupo_flashban";
...
#echo $grupoSel;
	...
	if($continuar){
		...
		$query = $myAdmin->conexion->query("SELECT * FROM ".PREFIJO."grupo_flashban ORDER BY kid_grupo DESC");#LIMIT ".$paginacion['LIMIT']
	}
...
<table class="layout" width="90%" align="center">
	<tr>
		...
		<a style="cursor:pointer" onclick="ajax_getData('<?php echo WEB_PATH; ?>admin/adm.flashgroups.php','<?php echo $page; ?>&new&grupo=<?php echo $grupoSel; ?>','groupContenedor','GET');" class="btn">
		<div class="inner">
			<div class="crnl">
			<div class="crnr">
	...
	foreach($arr as $reg){
		...
		#}else{
			...
			<tr class="<?php  echo $reg['visible']==0?' inactivo':''?>">
				...
				<td><a style="cursor:pointer" onclick="ajax_getData('<?php echo WEB_PATH; ?>admin/adm.flashgroups.php','<?php echo $page; ?>&fast=<?php echo $reg['visible']==1?'invisible':'visible'; ?>&id=<?php echo $reg['kid_grupo'] ?>','groupContenedor','GET');" title="Hacer <?php echo $reg['visible']==1?'invisible':'visible'; ?>"><img src="<?php echo WEB_PATH; ?>admin/img/ico/sem-<?php echo $reg['visible']==1?'vd':'gr'; ?>.gif" /></a></td>
				<td><a style="cursor:pointer" onclick="ajax_getData('<?php echo WEB_PATH; ?>admin/adm.flashgroups.php','<?php echo $page; ?>&edit&id=<?php echo $reg['kid_grupo'] ?>','groupContenedor','GET');" title="Editar tema"><img src="<?php echo WEB_PATH; ?>admin/img/ico/edit.png" /></a></td>
