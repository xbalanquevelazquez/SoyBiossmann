...
*/			
	...
	if(isset($_POST['r']['id']) && is_numeric($_POST['r']['id'])){
		$id = $_POST['r']['id'];
		if($myAdmin->conexion->update('como_dice',$_POST['res'],"WHERE kid_dice=$id",'HTML')){
...
$res = $myAdmin->conexion->fetch($myAdmin->conexion->query("SELECT * FROM como_dice WHERE kid_dice='".$_GET['id']."'"));
$res = $res[0];
#print_r($res);
...
<table class="form" cellspacing="0">
	<input type="hidden" name="r[id]" value="<?php echo $res['kid_dice']; ?>" />
	<tr>
		<th><label for="res[autor]">Autor</label></th>
		<td><input type="text" maxlength="200" size="45" name="res[autor]" id="autor" value="<?php echo $res['autor']; ?>" /></td>
	</tr>
	<tr>
	...
	<tr>
		<th><label for="res[autor]">Link</label></th>
		<td><input type="text" maxlength="200" size="45" name="res[link]" id="link" value="<?php echo $res['link']; ?>" /></td>
	</tr>
	<tr>
...
<script type="text/javascript">
	var obj = document.getElementById('res[autor]');
	obj.focus();
</script>
<?php } ?>
