function enviaAJAX(url){
	var data = Array();
	data[0] = Array('res[titulo]','texto','"Titulo"');
	if(validarFormulario(data)){
		var kid_link = document.getElementById('r[id]').value;
		...
		var icono = document.getElementById('res[icono]');
		icono = icono.options[icono.selectedIndex].value;
		...
		var vars='&r[id]='+kid_link+"&res[titulo]="+titulo+"&res[grupo]="+grupo+"&res[link]="+liga+"&res[icono]="+icono+"&res[posicion]="+posicion+"&res[selector]="+selector+"&res[visible]="+visible;
		//alert(url);
